...
namespace CrmCodeGenerator.VSPackage.Helpers
{
    public static class Status
    {
	    ...
	    private static readonly ConcurrentQueue<(string message, bool newLine)> messageQueue = new();

        public static void Update(string message, bool newLine = true)
        {
	        messageQueue.Enqueue((message, newLine));
			ProcessQueue();
        }
	    ...
	    private static void ProcessQueue()
        {
	        ...
	        while (messageQueue.TryDequeue(out var message))
	        {
		        ...
		        pane.OutputString(message.message);
